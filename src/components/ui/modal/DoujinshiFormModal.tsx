...
interface DoujinshiFormModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const DoujinshiFormModal: React.FC<DoujinshiFormModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  initialName = "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedName) {
      setError("Tên doujinshi không được để trống");
      return;
    }

    if (trimmedName.length < 2) {
      setError("Tên doujinshi phải có ít nhất 2 ký tự");
      return;
    }

    if (trimmedName.length > 255) {
      setError("Tên doujinshi không được vượt quá 255 ký tự");
      return;
    }
  ...
  const title = mode === "create" ? "Tạo doujinshi mới" : "Sửa tên doujinshi";
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[500px] p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {mode === "create"
            ? "Nhập tên doujinshi để tạo mới"
            : "Cập nhật tên doujinshi"}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-6">
          <label
            htmlFor="doujinshi-name"
            className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
          >
            Tên doujinshi <span className="text-red-500">*</span>
          </label>
          <input
            id="doujinshi-name"
            type="text"
            ...
            placeholder="Nhập tên doujinshi"
            disabled={isLoading}
