...
export const GroupFormModal: React.FC<GroupFormModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmedName) {
      setError("Tên nhóm dịch không được để trống");
      return;
    }

    if (trimmedName.length < 2) {
      setError("Tên nhóm dịch phải có ít nhất 2 ký tự");
      return;
    }

    if (trimmedName.length > 255) {
      setError("Tên nhóm dịch không được vượt quá 255 ký tự");
      return;
    }
  ...
  const title = mode === "create" ? "Tạo nhóm dịch mới" : "Sửa tên nhóm dịch";
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[500px] p-6">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-6">
          ...
          >
            Tên nhóm dịch <span className="text-red-500">*</span>
          </label>
          <input
            id="group-name"
            type="text"
